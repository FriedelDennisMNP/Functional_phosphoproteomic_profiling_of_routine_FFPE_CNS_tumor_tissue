...
pep_se<-pep_se[,pep_se$sample_id%in%gsub("_p","",metadata$original_id)]
colnames(pep_se)<-metadata$sample_id
SummarizedExperiment::colData(pep_se)<-S4Vectors::DataFrame(metadata[match(pep_se$sample_id,gsub("_p","",metadata$original_id)),])
...
saveRDS(mart,save_here(object_name="hsapiens_gene_ensembl_mart"))
