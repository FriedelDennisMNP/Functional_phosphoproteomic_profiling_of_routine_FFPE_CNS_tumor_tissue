...
dataset = paste0('/Figure2_PhosProt/')
library("rip")
library("proteoLab")
...
##### Load PTM data ###
ptm<-readRDS("./output/20260813_RRS_1296_PCF_Phospho//PTM_PRC_DEA/2026-08-19///ptm_se_prc.rds")
wp<-readRDS("./output/20260817_RR1296_PCF/WP_PRC_DEA/2026-08-20/ms_se_prc.rds")

datasets<-list("PTM"=ptm$imp,
               "WP"=wp$imp)

figure2A<-purrr::map(datasets,function(ms_imp){
...
ggsave(plot = figure2A[[1]],save_here(object_name = "Figure2A.pdf"),width = 6,height = 6)
...
ggsave(
  plot = enVo,
  save_here(object_name = "Figrue2B_PTMVolcano.pdf"),
  width = 10,
  height = 10
)
